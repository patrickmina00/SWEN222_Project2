...
public class RoomConstraintsShouldNotMoveTest {

	private ArrayList<RoomConstraint> roomConstraint = new ArrayList<RoomConstraint>();
	private Room testRoom;

	/**
	 * Sets up the roomConstraints and testRoom with the invalid state in item 2
	 * @throws Exception
	 */
	@Before
	public void setUp() throws Exception {
		...
		testRoom.addItem(itemManager.getItemWithIdAndState(2, ItemState.TRUE)); //Lever 2, IS SET TO TRUE!!
	...
	public void test() {
		...
		boolean didViolateConstraints = false;
		...
		for(RoomConstraint rConstraint : this.roomConstraint ){			
			...
			if(!rConstraint.passesConstraint(testRoom)){					
				...
				didViolateConstraints = true;
				
			}
		}
		
		assertTrue("No constraints were violated", didViolateConstraints);
	}
